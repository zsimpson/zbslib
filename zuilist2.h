...
struct ListItem {
	...
	int itemType;
	ListItem *parent;
		// WARNING WARNING WARNING: this parent pointer will get fucked if the ZTLVec in the ZUIList
		// has to realloc.  So you must ensure the initial alloc is enough to hold what you want, or
		// mods need to be made like I did to ZUIList in zuilist.h to make this into an index
	...
	ListItem() {
		...
		itemZui	=  0;
		itemType=  0;
		flags   = LI_SELECTABLE | LI_VISIBLE | LI_EXPANDED;
		parent  =  0;
	}
};
...
public:
	...
	// ZUIList
	void init();
	ListItem* addItem( void *item, int type=0, ListItem* parent=0 );
	...
	void setCheckboxState( ListItem *li, int state );
