...
//		*REQUIRED_FILES fft.cpp fft.h
//		*VERSION 1.0
//		+HISTORY {
//			Original code by Laurent de Soras( c ) 1999 from FFTReal.cpp
//			This is a modified version
//				Modified by Zack Booth Simpson 20 Feb 2004
//				Modified names of do_fft and do_ifft to match my naming conventions
//				Added class FFT as a general purpose class to do more common work
//				Changed the formatting to match ZBSLIB standards
//				Simplified exclusion defines
//			Notes from original license:
...
void FFTReal::doFFT( flt_t f [], const flt_t x [] ) const {
	if( _nbr_bits > 2 ) {
		flt_t *sf;
		flt_t *df;
			...
			for( pass = 3; pass < _nbr_bits; ++pass ) {
				...
				do {
					...
					for( i = 1; i < h_nbr_coef; ++ i ) {
						const flt_t c = cos_ptr [i];
							// cos( i*PI/nbr_coef );
						const flt_t s = cos_ptr [h_nbr_coef - i];
							// sin( i*PI/nbr_coef );
						flt_t v;
...
void FFTReal::doIFFT( const flt_t f [], flt_t x [] ) const {
	// General case
	if( _nbr_bits > 2 ) {
...
int FFTReal::getOrderedCoeffs( flt_t f[], flt_t reals[], flt_t imags[] ) {
	reals[0] = f[0];
	imags[0] =( flt_t )0.0;
	for( int i=1; i<_length/2-1; i++ ) {
		reals[i] = f[i];
		imags[i] = f[_length/2+i];
	}
	return _length/2-1;
}
...
static int fftNextPow2Bits( int x ) {
	int p = 1;
	for( int i=0; i<32; i++ ) {
		if( p >= x ) {
			return i;
...
int FFT::getNumFreqCoefsFromTime( int numTimeSamples ) {
	int pow2Bits = fftNextPow2Bits( numTimeSamples );
	int pow2Size = 1 << pow2Bits;
	return pow2Size / 2 + 1;
}

void FFT::alloc( int actualNumTimeSamples ) {
	clear();
	numTimeSamples = actualNumTimeSamples;
	pow2Bits = fftNextPow2Bits( numTimeSamples );
	pow2Size = 1 << pow2Bits;
	pow2Time  = (float *)malloc( sizeof(float) * pow2Size );
	pow2Freq = (float *)malloc( sizeof(float) * pow2Size );
	memset( pow2Time, 0, sizeof(float) * pow2Size );
	memset( pow2Freq, 0, sizeof(float) * pow2Size );
}
...
void FFT::clear() {
	...
	if( pow2PowerSpectrum && powerSpectrumOwner ) {
		free( pow2PowerSpectrum );
		pow2PowerSpectrum = 0;
	}

	if( pow2PhaseSpectrum ) {
		free( pow2PhaseSpectrum );
		pow2PhaseSpectrum = 0;
	}
...
void FFT::fft() {
	assert( sizeof(float) == sizeof(flt_t) );
	FFTReal fftReal( pow2Size );
	fftReal.doFFT( pow2Freq, pow2Time );
}

void FFT::ifft() {
	assert( sizeof(float) == sizeof(flt_t) );
	FFTReal fftReal( pow2Size );
	fftReal.doIFFT( pow2Freq, pow2Time );
}

float *FFT::hanningLookup = 0;
int FFT::hanningLookupSize = 0;

void FFT::clearStaticCachedLookupTables() {
	if( hanningLookup ) {
		free( hanningLookup );
		hanningLookup = 0;
		hanningLookupSize = 0;
...
float *FFT::getTimeCenterPtr() {
	assert( pow2Time );
	int offset = (pow2Size-numTimeSamples) / 2;
	return &pow2Time[offset];
}

void FFT::smoothTime() {
	if( numTimeSamples != pow2Size ) {
		int halfReal = numTimeSamples / 2;
		int halfPow2 = pow2Size / 2;
		assert( pow2Bits < 32 );

		if( hanningLookupSize != numTimeSamples ) {
			// CREATE the Hanning window lookup table
			hanningLookup = (float *)malloc( sizeof(float) * halfReal );
			hanningLookupSize = numTimeSamples;
			for( int i=0; i<halfReal; i++ ) {
				#define PI2F (2.f * 3.14159265358979323846f)
				hanningLookup[i] = 0.5f + 0.5f * cosf( PI2F * (float)i / numTimeSamples );
			}
		}

		for( int i=0; i<halfReal; i++ ) {
			pow2Time[halfPow2-i-1] *= hanningLookup[i];
			pow2Time[halfPow2+i  ] *= hanningLookup[i];
		}
	}
}

void FFT::loadTimeSamples( float *signal ) {
	assert( pow2Time );

	int offset = (pow2Size-numTimeSamples) / 2;
	memset( pow2Time, 0, sizeof(float) * offset );
	memcpy( &pow2Time[offset], signal, numTimeSamples * sizeof(float) );
	memset( &pow2Time[offset+numTimeSamples], 0, sizeof(float) * (pow2Size-offset-numTimeSamples) );

	smoothTime();
}

float *FFT::computePowerSpectrum() {
	if( !pow2PowerSpectrum ) {
		pow2PowerSpectrum = (float *)malloc( sizeof(float) * pow2Size );
		powerSpectrumOwner = 1;
	}
	int numCoef = getNumFreqCoefs();
	for( int i=0; i<numCoef; i++ ) {
		float real = getFreqCoefReal(i);
		float imag = getFreqCoefImag(i);
		pow2PowerSpectrum[i] = real*real + imag*imag;
	}
	return pow2PowerSpectrum;
}

float *FFT::computePhaseSpectrum() {
	if( !pow2PhaseSpectrum ) {
		pow2PhaseSpectrum = (float *)malloc( sizeof(float) * pow2Size );
	}
	int numCoef = getNumFreqCoefs();
	for( int i=0; i<numCoef; i++ ) {
		float real = getFreqCoefReal(i);
		float imag = getFreqCoefImag(i);
		pow2PhaseSpectrum[i] = (float)atan2f( imag, real );
	}
	return pow2PhaseSpectrum;
}
...
FFT::FFT() {
	...
	pow2PowerSpectrum = 0;
	pow2PhaseSpectrum = 0;
	powerSpectrumOwner = 0;
}

FFT::FFT( int actualNumTimeSamples ) {
	numTimeSamples = 0;
	pow2Size = 0;
	pow2Bits = 0;
	pow2Time = 0;
	...
	pow2PhaseSpectrum = 0;
	powerSpectrumOwner = 0;
	alloc( actualNumTimeSamples );
}

FFT::FFT( float *timeSignal, int actualNumTimeSamples ) {
	numTimeSamples = 0;
	pow2Size = 0;
	pow2Bits = 0;
	pow2Time = 0;
...
float *fftPowerSpectrum( float *signal, int count, int stride, int &outputSize ) {
	...
	fft.zeroTime();
	float *dst = fft.getTimeCenterPtr();
	char *src = (char *)signal;
	...
	fft.powerSpectrumOwner = 0;
		// Tell the fft not to free this memory so that it can be returned here
	outputSize = fft.getNumFreqCoefs();
	return fft.pow2PowerSpectrum;
}
